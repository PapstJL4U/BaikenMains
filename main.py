...
def get_csv_via_panda():
    try:
        ...
        print(data_format.index)
        chars = data_format.keys()[3:-1]

        fig1 = px.bar(data_format, x=chars, y="date", orientation='h', hover_data = { 'date' : False},
                     title="Character Usage in %")
        fig1.update_layout( barnorm = "percent")
        fig1.show()

        fig2 = px.bar(data_format, x=chars, y="date", orientation='h', hover_data = { 'date' : False},
                      title="Charater Usage", color_discrete_sequence=colour.css1,)
        fig2.show()
    except:
        raise
